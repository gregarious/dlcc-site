...
class EventProcessor extends ModelProcessor {
	public $typeName = 'hotel';
	public $typeUrl = 'hotels.php';
	public $typeDbName = 'hotel';
	...
	function createModel() {
		...
		$tpl = "INSERT INTO `$this->typeDbName` (name, address, lat, lng, phone) VALUES ('%s', '%s', '%s', '%s', '%s')";
		return runQuery($tpl,
						$_POST['name'], 
						$_POST['address'], 
						$_POST['lat'], 
	...
	function saveModel($id, $object) {
		...
		$tpl = "UPDATE `$this->typeDbName` SET name = '%s', address = '%s', lat = '%s', lng = '%s', phone = '%s' WHERE id = %s";
		return runQuery($tpl,
						$_POST['name'], 
						$_POST['address'], 
						$_POST['lat'], 
